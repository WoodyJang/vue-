...
function isUndef (v) {             //是否未定义
  return v === undefined || v === null
}

function isDef (v) {                 //是否已定义
  return v !== undefined && v !== null
}
 ...
 */
function isPrimitive (value) {        // 是否为简单数据类型
  return (
    typeof value === 'string' ||
    typeof value === 'number' ||
    // $flow-disable-line
...
function looseEqual (a, b) {
  ...
  if (isObjectA && isObjectB) {
    try {
      ...
      if (isArrayA && isArrayB) {                          //判断两个数组是否相等
        return a.length === b.length && a.every(function (e, i) {
          return looseEqual(e, b[i])
        })
      } else if (!isArrayA && !isArrayB) {                  //判断2个对象是否相等
...
initMixin(Vue$3);          //初始化的入口，各种初始化工作
stateMixin(Vue$3);         //数据绑定的核心方法，包括常用的$watch方法
eventsMixin(Vue$3);        //事件的核心方法，包括常用的$on，$off，$emit方法
lifecycleMixin(Vue$3);     //生命周期的核心方法
renderMixin(Vue$3);        //渲染的核心方法，用来生成render函数以及VNode
...
function initGlobalAPI (Vue) {
  ...
  initMixin$1(Vue);              //【Vue.mixin】 通过initMixin方法定义
  initExtend(Vue);             //【Vue.extend】通过initExtend方法定义
  initAssetRegisters(Vue);
}

initGlobalAPI(Vue$3);      //2、调用initGlobalAPI,定义全局资源, 将 Vue 构造函数作为参数，传递给 initGlobalAPI 方法，该方法来自 ./global-api/index.js 文件
...
Vue$3.version = '2.5.13';   //该值最终会被替代 scripts/config.js 文件, genConfig 方法
...
function createPatchFunction (backend) {
  ...
  function patchVnode (oldVnode, vnode, insertedVnodeQueue, removeOnly) {
    if (oldVnode === vnode) {         //如果oldVnode跟vnode完全一致，那么不需要做任何事情
      return
    }
    ...
    if (isTrue(vnode.isStatic) &&
      ...
      (isTrue(vnode.isCloned) || isTrue(vnode.isOnce))      //如果oldVnode跟vnode都是静态节点，且具有相同的key，当vnode是克隆节点或是v-once指令控制的节点时，只需要把oldVnode.elm和oldVnode.child都复制到vnode上，也不用再有其他操作
    ) {
  ...
  return function patch (oldVnode, vnode, hydrating, removeOnly, parentElm, refElm) {
    if (isUndef(vnode)) {
      if (isDef(oldVnode)) { invokeDestroyHook(oldVnode); }            //vnode不存在，oldvnode存在，销毁老节点
      return
    }
    ...
    if (isUndef(oldVnode)) {
      ...
      isInitialPatch = true;
      createElm(vnode, insertedVnodeQueue, parentElm, refElm);          //创建新节点
    } else {
      var isRealElement = isDef(oldVnode.nodeType);
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        // patch existing root node
        patchVnode(oldVnode, vnode, insertedVnodeQueue, removeOnly);    //同一个节点，调用patchVnode方法
      } else {
        if (isRealElement) {
